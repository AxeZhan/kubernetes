...
func TestValidateClaimStatusUpdate(t *testing.T) {
	...
	for name, scenario := range scenarios {
		t.Run(name, func(t *testing.T) {
			...
			scenario.oldClaim.ResourceVersion = "1"
			claim, oldClaim := scenario.update(scenario.oldClaim.DeepCopy()), scenario.oldClaim
			opts := ValidationOptionsForResourceClaimStatus(claim, oldClaim)
			errs := ValidateResourceClaimStatusUpdate(claim, oldClaim, opts)
			assert.Equal(t, scenario.wantFailures, errs)
		})
	}
}
